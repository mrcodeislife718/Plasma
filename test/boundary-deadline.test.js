...
test('Plasma rejects adapter work that outlives the boundary deadline', async () => {
  ...
  registry.register('slow', {
    ...
    async invoke(_call, { signal } = {}) {
      ...
      return delay(100, 'late');
    }
  });
  const call = createBoundaryCall({ module:'slow', member:'run', deadline:Date.now() + 20 });
...
test('Plasma applies the same deadline to policy evaluation', async () => {
  const registry = new AdapterRegistry({ policy: async () => delay(100) });
  let invoked = false;
  registry.register('target', {
    capabilities: () => ({ language:'test' }),
    async invoke() { invoked = true; return true; }
  });
  const call = createBoundaryCall({ module:'target', member:'run', deadline:Date.now() + 20 });
...
test('Plasma completes calls normally when they finish inside the deadline', async () => {
  ...
  const call = createBoundaryCall({ module:'fast', member:'run', deadline:Date.now() + 1000 });
